...
export default function TripRegistration() {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* Route preview */}
      {(routeLoading || route) && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--accent-softer)', borderRadius: 10, border: '1px solid rgba(74,222,85,0.2)' }}>
          {routeLoading
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 8 }}>
          {vehicles.map(v => (
            <button key={v.value} type="button" onClick={() => set('vehicle_type', v.value)} style={{
              ...
              border: `1px solid ${form.vehicle_type === v.value ? 'rgba(74,222,85,0.5)' : 'var(--border)'}`,
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {toggles.map(({ key, label }) => (
            <button key={key} type="button" onClick={() => set(key, !form[key])} style={{
              ...
              border: `1px solid ${form[key] ? 'rgba(74,222,85,0.4)' : 'var(--border)'}`,
