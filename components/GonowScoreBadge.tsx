...
export function GonowScoreCard({
  result, ratingAvg, ratingCount, bankidVerified, completedTrips, nextRequirements,
}: GonowScoreCardProps) {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        <div style={{ flexShrink: 0, display: 'flex', alignItems: 'baseline', gap: 4 }}>
          ...
          <span style={{ fontSize: '0.72rem', color: 'var(--muted)', fontWeight: 500 }}>/100</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flex: 1, justifyContent: 'flex-end' }}>
          <span style={{ fontSize: '0.6rem', color: 'var(--muted)', marginRight: 6, whiteSpace: 'nowrap', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Gonow Score™</span>
          {TIERS.map((t, i) => {
            const isPassed  = score >= t.max
            const isCurrent = t.key === tier.key
            return (
              <div key={t.key} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 999, background: isPassed || isCurrent ? `${t.color}20` : 'transparent', border: `1px solid ${isPassed || isCurrent ? t.color + '55' : 'rgba(0,0,0,0.08)'}` }}>
                  <span style={{ fontSize: '0.7rem' }}>{t.emoji}</span>
                  {isCurrent && <span style={{ fontSize: '0.62rem', fontWeight: 700, color: t.color }}>{t.label}</span>}
                </div>
                {i < TIERS.length - 1 && (
                  <div style={{ width: 12, height: 1, background: 'rgba(0,0,0,0.12)' }} />
                )}
              </div>
            )
          })}
      ...
      {nextTier && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: '0.68rem', color: 'var(--muted)', whiteSpace: 'nowrap', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--text)', flexShrink: 0 }}>{score}/{nextTier.min}p</span>
        </div>
      )}
      ...
      <div>
        <p style={{ fontSize: '0.6rem', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Poängfördelning</p>
          ...
          {bars.map(bar => (
            <div key={bar.label} style={{ display: 'flex', flexDirection: 'column', gap: 5, opacity: (bar as { locked?: boolean }).locked ? 0.4 : 1, minWidth: 72, flex: 1 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '0.65rem', fontWeight: 700, color: 'var(--text)' }}>{bar.pts}/{bar.max}</span>
              </div>
      ...
      {nextTier && nextRequirements.length > 0 && (
        <>
          <div style={{ height: 1, background: 'rgba(0,0,0,0.06)' }} />
          <div>
            <p style={{ fontSize: '0.6rem', color: 'var(--muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>
              Vad krävs för <span style={{ color: nextTier.color }}>{nextTier.label}</span>?
            </p>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {nextRequirements.map(req => (
                <div key={req.label} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 10, background: req.done ? 'rgba(34,197,94,0.08)' : 'rgba(0,0,0,0.05)', border: `1px solid ${req.done ? 'rgba(34,197,94,0.25)' : 'rgba(0,0,0,0.08)'}` }}>
                  <span style={{ fontSize: '0.78rem' }}>{req.done ? '✅' : req.icon}</span>
                  <div>
                    <p style={{ fontSize: '0.68rem', fontWeight: 600, color: req.done ? 'var(--muted)' : 'var(--text)', textDecoration: req.done ? 'line-through' : 'none', margin: 0 }}>{req.label}</p>
                    {!req.done && req.target > 1 && (
                      <p style={{ fontSize: '0.62rem', color: 'var(--muted)', margin: '2px 0 0' }}>{req.current}/{req.target}{req.unit ? ' ' + req.unit : ''}</p>
                    )}
                    {!req.done && req.target === 1 && req.unit === '' && (
                      <p style={{ fontSize: '0.62rem', color: 'var(--muted)', margin: '2px 0 0' }}>Krävs</p>
                    )}
                  </div>
                </div>
              ))}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, paddingTop: 2 }}>
        {ratingCount > 0 && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 10px', borderRadius: 999, background: 'var(--surface)', border: '1px solid var(--border)', fontSize: '0.68rem', fontWeight: 600, color: 'var(--text)' }}>
        ...
        {bankidVerified && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 10px', borderRadius: 999, background: 'rgba(52,211,153,0.1)', border: '1px solid rgba(52,211,153,0.25)', fontSize: '0.68rem', fontWeight: 600, color: '#059669' }}>
