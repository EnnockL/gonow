...
import { ShoppingBag, MapPin, FileText, Store, ArrowRight, CheckCircle2, Package } from 'lucide-react'

const POPULAR_STORES = [
  { name: 'IKEA', locations: 'Kungens Kurva, Bäckebol, Hamarby' },
...
export default function HamtaPage() {
  const [form, setForm] = useState({
    store_name: '', store_address: '', order_reference: '', delivery_address: '', city: '',
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await fetch('/api/ai-parse', {
      ...
      body: JSON.stringify({
        message: `Hämta min beställning från ${form.store_name} (${form.store_address}), ordernummer ${form.order_reference}, leverera till ${form.delivery_address} i ${form.city}.`,
      }),
    })
    setLoading(false)
    setSubmitted(true)
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 24px' }}>
        <div style={{ textAlign: 'center', maxWidth: 380 }}>
          ...
          <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.02em', marginBottom: 10 }}>
            Upphämtning beställd!
          </h2>
          <p style={{ color: 'var(--muted)', fontSize: '0.875rem', marginBottom: 24, lineHeight: 1.6 }}>
            Vi matchar dig med en bärare som passerar {form.store_name || 'butiken'}. Du notifieras när någon tackar ja.
          </p>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 24px' }}>
        ...
        <div style={{ paddingTop: 32, paddingBottom: 48 }}>
          ...
          <h1 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.75rem)', fontWeight: 700, letterSpacing: '-0.03em', color: 'var(--text)', marginBottom: 12, lineHeight: 1.1 }}>
            Hämta din order.<br />Utan att röra dig.
          </h1>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input required placeholder="t.ex. IKEA, Biltema" value={form.store_name}
                ...
                <div>
                  ...
                  <input required placeholder="t.ex. Kungens Kurva, Stockholm" value={form.store_address}
              ...
              <div>
                ...
                <input required placeholder="t.ex. IKEA-20260601-4521" value={form.order_reference}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input required placeholder="t.ex. Storgatan 12" value={form.delivery_address}
                    onChange={(e) => set('delivery_address', e.target.value)}
                    onFocus={focusIn} onBlur={focusOut} style={inputStyle} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '0.72rem', color: 'var(--muted)', marginBottom: 6, fontWeight: 500 }}>Stad</label>
                  <input required placeholder="t.ex. Sundbyberg" value={form.city}
              ...
              <button type="submit" disabled={loading} className="btn-primary"
                style={{ width: '100%', borderRadius: 10, padding: '12px 0', marginTop: 4 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div className="card-sm">
              ...
              {POPULAR_STORES.map((s) => (
                <div
                  key={s.name}
                  onClick={() => set('store_name', s.name)}
                  style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '8px 0', borderBottom: '1px solid var(--border)', cursor: 'pointer', transition: 'opacity 0.15s' }}
            ...
            <div className="card-sm" style={{ background: 'var(--success-soft)', borderColor: 'var(--success-border)' }}>
              <p style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--success)', marginBottom: 6 }}>✓ Inget postkontor</p>
              <p style={{ fontSize: '0.73rem', color: 'var(--muted)', lineHeight: 1.6 }}>
                Slipp köa på PostNord. Betalning frigörs efter leverans — alltid trygg handel.
              </p>
            </div>
          </div>
        </div>
